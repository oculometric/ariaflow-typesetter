...
void UITextEditor::checkInput(Window* w, glm::vec2& position, glm::vec2& size)
{
    ...
    change = size.x - glm::clamp(size.x - change, editor_min_width, static_cast<float>(w->getSize().x));
    ...
    change             = size.y - glm::clamp(size.y - change, editor_min_width,
                                      static_cast<float>(w->getSize().y) - UIRootMenu::getHeight());
    ...
    change2           = size - glm::clamp(size - change2, { editor_min_width, editor_min_width },
    ...
    change2.y         = size.y - glm::clamp(size.y - change2.y, editor_min_width,
                                     static_cast<float>(w->getSize().y) - UIRootMenu::getHeight());
    ...
    size = glm::clamp(size, { editor_min_width, editor_min_width },
        glm::vec2(w->getSize()) - glm::vec2{ 0, UIRootMenu::getHeight() });

    // TODO: snapping?
    // TODO: window resize behaviour
